...
def todo_help(message,comm):
    ...
    "noticetime: ToDo機能には期限の迫ったタスクを通知で知らせる機能があります。noticetimeは残りの通知回数です\n"\
    "userid（内部データ）:タスクの持ち主を示す個別識別番号です。S2340A7K6Q4というように表示されます\n"\
    "deleted:そのタスクのidが使われているなら0、使われていないなら1が入力されます。deletedが1のタスクは非表示になります\n"\
